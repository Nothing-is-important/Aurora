...
### Aurora 原生客户端（aurora-native）
- 引擎进程内嵌入：Electron 主进程直接 boot DeepSeek Harness（无子进程，65 个引擎服务直连）
- 自绘苹果式玻璃 UI：会话列表 / 流式对话 / 工具步骤卡 / 思维链折叠 / 统计条
- 模型供应商制：API Key → 测速并获取模型列表（名称/上下文/上限）；模型菜单按密钥过滤
- DSH 数据目录可切换（复用已有部署的密钥与会话）
- 命令面板（Ctrl+K）+ 快捷键 + 消息编辑/重新生成（引擎会话分叉）+ 导出 MD/JSON + 23 个提示词模板 + 深浅色
- 动态插件面板（官方 dynamicCordisRunner，定义/运行/停止/删除）+ MCP 组合配置 + 知识库 RAG（BM25 引擎工具 kb_search）
- 桌面体验：托盘常驻 / 窗口状态记忆 / 单实例 / 版本显示
- 内置 dsh 运行时（单文件 zip 首启解压，共享自愈），40+ 断言冒烟审计

### Aurora DSH 壳客户端（dsh-shell）
- 原生窗口包裹官方 dsh web：窗口/托盘/快捷键/通知/窗口状态/崩溃自动重启/缺失引导
- 内置 dsh 运行时 + 便携 node；图标改用 Aurora 官方图标；安装秒级（单 zip）

### 其它
- 版本统一 1.0.0；README/changelog 重写；dsh 嵌入可行性报告随附
